...
该脚本合并了proxy server和benchmark客户端功能，消除网络通信瓶颈。
复用现有的数据集加载逻辑，添加proxy server的KV transfer参数生成。

主要功能：
1. 加载mooncake数据集
2. 复制proxy server的KV transfer参数生成和请求处理
3. 直接发送请求到decode instance，绕过proxy server
"""
...
def create_argument_parser():
    """创建参数解析器"""
    parser = argparse.ArgumentParser(description="Integrated PD separation benchmark")
...
async def main():
    ...
    dataset = MooncakeDataset(random_seed=args.seed, dataset_path=args.dataset_path)
    input_requests = dataset.sample(tokenizer=tokenizer, num_requests=args.num_prompts)

    # 创建processor并运行benchmark
